...
from extensions.serializers import ExtentionSerializer, GetExtensionSerializer


class ExtensionView(ModelViewSet):
    """
    ViewSet for Extention model which provides CRUD operations.
    """
    queryset = Extension.objects.all()
    serializer_class = ExtentionSerializer
    list_serializer_class = GetExtensionSerializer
    ...
    def get_serializer_class(self):
        ...
        return super(ExtensionView, self).get_serializer_class()
...
class ExtensionReview(APIView):
    ...
    def post(self, request, id, *args, **kwargs):
        try:
            extension_object = Extension.objects.get(id=id)
        except Extension.DoesNotExist:
            return Response({"message": "Extension does not exist"}, status=404)

        review_objects = Review.objects.filter(user=request.user)
        all_reviews = extension_object.reviews.all()
        if review_objects.intersection(all_reviews):
            return Response({"message": "You have already reviewed this Extension"}, status=400)

        serializer = self.performer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.validated_data['user'] = request.user
        review = serializer.save()
        extension_object.reviews.add(review)
        extension_object.save()
    ...
    def patch(self, request, id,*args, **kwargs):
        try:
            ...
            if review_object:
                extension_obj = review_object.extension_reviews.all()[0]
